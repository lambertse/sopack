...
| 6 | **O-MVLL on sopack's own code** (`stub/omvll_config_wb.py`) | ✅ shipped | Was the single biggest hole: `--omvll` reached only the vendored `libwbcrypto.a`, while `MANIFEST.txt` claimed the provider was obfuscated. Took **two** fixes: wiring the plugin in, then correcting a config method name the plugin silently ignored ([S8](./STATIC-ANALYSIS-REVIEW.md)) |
| 6b | **Version scripts on both wbaes links** | ✅ shipped | O-MVLL **promotes the linkage** of what it transforms, so `-fvisibility=hidden` does not survive the pass. Without `{ local: *; };` an obfuscated helper exports `sopk_rt_ctor`/`self_cb`/`tgt_cb`/`sopk_wipe` — obfuscation handing back a labelled map |
| 7 | **A mechanical obfuscation gate** (`scripts/check_obfuscated.sh`) | ✅ shipped | Measures the artifact, so the manifest's claim cannot silently be false. **Structural** (O-MVLL's outlined `name.N` siblings), never a threshold: the same source measured 613/712/1247/2223 across plugin versions and one config string. Runs pre-strip; needs `$NDK`; exit 2 = "cannot tell", never a pass |
